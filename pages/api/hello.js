...
// const groups = will.reduce((groups, item) => {
//   const group = (groups[item.wilaya_name_ascii] || []);
//   group.push(item);
...
const groupBy = (arr, groupFn) =>
  arr.reduce(
    (grouped, obj) => ({
      ...grouped,
      [String(groupFn(obj))]: [...(grouped[groupFn(obj)] || []), {key:obj.commune_name_ascii, value:obj.commune_name_ascii}],
    }),
    []
  );
...
const groupedByNameLengths = groupBy(will, (person) => person.wilaya_name_ascii);

for (const [key, value] of Object.entries(groupedByNameLengths)) {
    willaya.push({key: key, value:key})
}

// const groupedByNameLengths1 = []
// for (const [key, value] of Object.entries(groupedByNameLengths)) {

//     console.log(`${key}: ${value}`);
//   }

console.log(groupedByNameLengths);
